...
public class TcpClient(string host = "localhost", int port = 8888) : IDisposable {
    ...
    public async Task<Response> SendRequestAsync(Request request) {
        ...
        try {
            ...
            int    bytesRead      = await _stream.ReadAsync(responseBuffer);
            ...
            var response = JsonSerializer.Deserialize<Response>(jsonResponse, options);

            // 处理错误响应
            if (response.code == "error") {
                try {
                    var    jsonElement  = (JsonElement)response.payload;
                    string errorMessage = jsonElement.GetProperty("message").GetString();
